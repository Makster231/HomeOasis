...
const js_plugins = [
  path.app.jsComponents + "variables.js",
  path.app.jsComponents + "nav-pages.js",
  path.app.jsComponents + "particles.js",
  path.app.jsComponents + "nav.js",
  path.app.jsComponents + "slider.js",
...
gulp.task("html:dev", done => {
  //compile .html pages in ./app (root);
  gulp
...
gulp.task(
  "build:dev", //build for DEV env
  gulp.series("html:dev", "style:dev", "js:dev"),
  done => {
    done();
  }
);
...
gulp.task("style:prod", done => {
  //recompile styles from ./app/scss
  gulp
    .src(path.app.scss)
    .pipe(sass({ outputStyle: "compact" }).on("error", sass.logError))
    .pipe(
...
gulp.task("js:prod", done => {
  // recompile scripts from ./app/js/components
  gulp
    .src(js_plugins)
    .pipe(concat("custom.js"))
    .pipe(
...
gulp.task(
  "build:prod", //build for PROD env
  gulp.series("clean:dist", "html:prod", "style:prod", "js:prod"),
  done => {
    done();
  }
);
